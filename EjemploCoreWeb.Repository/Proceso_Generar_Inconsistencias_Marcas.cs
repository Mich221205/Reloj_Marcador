...
namespace EjemploCoreWeb.Repository
{
    public class Proceso_Generar_Inconsistencias_Marcas
    {
        ...
        // GENERAR INCONSISTENCIAS DE MARCAS
        ...
        // Obtener funcionarios según área o ID de usuario
        public async Task<IEnumerable<Usuario>> ObtenerFuncionariosAsync(int? areaId = null, int? usuarioId = null)
        {
            using var connection = _dbConnectionFactory.CreateConnection();
            connection.Open();
            var sql = @"
                SELECT u.ID_Usuario, u.Identificacion, u.Nombre, u.Apellido_1, u.Apellido_2
                FROM usuario u
        ...
        // Obtener horario detallado para un usuario específico y día de la semana
        ...
        // Obtener marcas de un usuario en una fecha específica
        public async Task<IEnumerable<Marca>> ObtenerMarcasAsync(int idUsuario, DateTime fecha)
        {
            using var connection = _dbConnectionFactory.CreateConnection();
            connection.Open();
            var sql = @"SELECT * FROM marca WHERE ID_Usuario = @IdUsuario
        ...
        public async Task<bool> EsDiaExcluidoAsync(int idUsuario, DateTime fecha)
        {
            ...
            // Vacaciones
            var vac = await connection.ExecuteScalarAsync<int?>(
            ...
            if (vac.HasValue) return true;

            // Permisos
            var perm = await connection.ExecuteScalarAsync<int?>(
                "SELECT 1 FROM permisos WHERE ID_Usuario = @IdUsuario AND Estado='Aprobado' AND @Fecha BETWEEN Fecha_Inicio AND Fecha_Fin;",
                new { IdUsuario = idUsuario, Fecha = fecha });
            if (perm.HasValue) return true;

            return false;
        }

        // Guardar inconsistencias detectadas en la base de datos utilizando transacciones
        ...
        // Registrar en la bitácora el resumen del proceso
